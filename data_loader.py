...
class ChnDataset(ListDataset):
    @staticmethod
    def load_data(filename):
        dataset = []
        data = pd.read_csv(filename, sep='\t')
        for line in data.iloc:
            dataset.append((line['text_a'], str(line['label'])))
        return dataset
...
if __name__ == "__main__":
    ...
    max_len = 512
    bert_path = './model_hub/chinese-bert-wwm-ext'
    tokenizer = BertTokenizer.from_pretrained(bert_path)
    train_dataset = LAWSDataset(file_path='./data/data.txt')
    ...
    collate = Collate(tokenizer=tokenizer, max_len=max_len, tag2id=tag2id)
    batch_size = 2
